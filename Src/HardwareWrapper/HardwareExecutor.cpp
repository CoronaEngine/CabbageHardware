...
static void incExec(uintptr_t id){ if(id) gExecutorStorage.write(id,[](ExecutorWrap &w){ ++w.refCount; }); }
static void decExec(uintptr_t id){ if(!id) return; bool destroy=false; gExecutorStorage.write(id,[&](ExecutorWrap &w){ if(--w.refCount==0){ delete w.impl; w.impl=nullptr; destroy=true; } }); if(destroy) gExecutorStorage.deallocate(id); }
...
HardwareExecutor::~HardwareExecutor()
{ if(executorID) decExec(*executorID); }
...
{
 ExecutorWrap otherWrap; gExecutorStorage.read(*other.executorID, [&](const ExecutorWrap &w){ otherWrap = w; });
