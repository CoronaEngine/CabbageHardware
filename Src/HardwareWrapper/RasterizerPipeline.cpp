...
static void incRaster(uintptr_t id){ if(id) gRasterizerPipelineStorage.write(id,[](RasterizerPipelineWrap &w){ ++w.refCount; }); }
static void decRaster(uintptr_t id){ if(!id) return; bool destroy=false; gRasterizerPipelineStorage.write(id,[&](RasterizerPipelineWrap &w){ if(--w.refCount==0){ delete w.impl; w.impl=nullptr; destroy=true; } }); if(destroy) gRasterizerPipelineStorage.deallocate(id); }
...
RasterizerPipeline::~RasterizerPipeline()
{ if(rasterizerPipelineID) decRaster(*rasterizerPipelineID); }
...
{ HardwareImage img; gRasterizerPipelineStorage.read(*rasterizerPipelineID, [&](const RasterizerPipelineWrap &w){ img = w.impl->getDepthImage(); }); return img; }

std::variant<HardwarePushConstant, HardwareBuffer, HardwareImage> RasterizerPipeline::operator[](const std::string &resourceName)
{ std::variant<HardwarePushConstant, HardwareBuffer, HardwareImage> r; gRasterizerPipelineStorage.read(*rasterizerPipelineID, [&](const RasterizerPipelineWrap &w){ r = (*w.impl)[resourceName]; }); return r; }

RasterizerPipeline &RasterizerPipeline::operator()(uint16_t width, uint16_t height)
{ gRasterizerPipelineStorage.read(*rasterizerPipelineID, [&](const RasterizerPipelineWrap &w){ (*w.impl)(width,height); }); return *this; }

RasterizerPipeline &RasterizerPipeline::record(const HardwareBuffer &indexBuffer)
{ gRasterizerPipelineStorage.read(*rasterizerPipelineID, [&](const RasterizerPipelineWrap &w){ w.impl->record(indexBuffer); }); return *this; }

//供执行器访问实现
RasterizerPipelineVulkan *getRasterizerPipelineImpl(uintptr_t id)
{
 RasterizerPipelineVulkan *ptr=nullptr; gRasterizerPipelineStorage.read(id, [&](const RasterizerPipelineWrap &w){ ptr = w.impl; }); return ptr;
}
