...
//struct MeshFrame
//{
//    uint64_t frameId{0}; // 单调递增帧号，由 mesh 线程生成。
...
//struct RenderFrame
//{
//    uint64_t frameId{0}; // 对应的 mesh 帧号。
//    Backend backend{Backend::EDSL}; // EDSL 或 GLSL。
//    HardwareImage *outputImage{nullptr}; // 最终显示的图像资源。
//    HardwareExecutor *executorRef{nullptr}; // 对应提交器，display 线程会先 wait 再 present。
//    Clock::time_point submitTimestamp{}; // render 提交完成时间，用于统计延迟。
//};
//
...
//    virtual std::shared_ptr<const void> meshTick(uint64_t frameId,
//                                                 Clock::time_point now,
//                                                 std::string &errorMessage) = 0;
//    // EDSL 渲染钩子：消费 MeshFrame，并向 executor 录制/提交命令。
//    virtual bool renderEDSLTick(const MeshFrame &meshFrame,
//                                HardwareExecutor &executor,
//                                const HardwareImage &outputImage,
//                                std::string &errorMessage) = 0;
//    // GLSL 渲染钩子：消费 MeshFrame，并向 executor 录制/提交命令。
//    virtual bool renderGLSLTick(const MeshFrame &meshFrame,
//                                HardwareExecutor &executor,
//                                const HardwareImage &outputImage,
//                                std::string &errorMessage) = 0;
//    // Display 钩子：在 display 线程每次呈现后触发，可用于场景级统计或调试。
//    virtual void displayTick(const RenderFrame &renderFrame) = 0;
//    // 场景资源释放。
